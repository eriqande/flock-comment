...
membership to each of $K$ clusters, scaled to sum to one) for a run belonging to the longest plateau (see below).
...
\subsection*{Results} 
Run times were relatively short for both programs. All runs of {\sc flock} 
...
486 minutes and averaged 80.8 minutes to complete each of the six groups of runs. Increasing
...
We did not observe major differences among the 
allocation of individuals into K clusters between each program. All six groups 
of {\sc flock} runs for $K = 2$ to $K=4$ yielded nearly identical clustering of individuals 
(See supplementary materials) none of which differed substantially from results obtained with 
{\sc structure} (Fig.~\ref{fig:qplots}). Of the 'best runs' for the six groups for $K=2$, two  
...
$K$ increases so does the number of solutions
that {\sc flock} finds (Fig.~\ref{fig:qplots}). For all groups of runs for $K\geq 2$, no plateaus were observed - all initial 
allocations resulted in solutions that differed. These solutions, however, are surprisingly similar to
the cluster membership posterior means identified by {\sc structure}. Both programs identify clusters 
...
lengths being greater than 6. 
\hrule

Interpreting the results from {\sc structure} is often accomplished by 
