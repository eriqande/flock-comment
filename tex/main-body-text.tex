...
the number of genetic clusters in a collection $K$ has drastically exanded
in the past decade to aid in the fields of evolution, epidemiology, and conservation
genetics. By virtute of this exanding desire to identify $K$ there has been a push to 
develop methods that can accurately infer $K$ while at the same time be easy to run
both in terms of proccess time and user interface. Naturally, a program claiming
...
a restrictured case of the simulated annealing algorthim to find the Bayesian 
...
It is not surprising then, that because of the similaries in the algortims implemented that 
we obsered  
...
One of the major claimed advantages of {\sc flock} over methods for solving the 
...
(1) the number of individuals in each collection and (2) the lenght of runs for each program in each comparision. 
In their comparison, the 
