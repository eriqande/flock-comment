...
Data sets were simulated (code provided on GitHub repository 
https://github.com/eriqande/flock-comment)
and evaluated in the programs {\sc flock} and {\sc structure} 
for the known value ($K=5$). Each dataset was composed of five populations of 
sizes 250, 250, 400, 550, and 550 comprised of individuals 
with 15 loci and an average pairwise $F_{ST} = 0.03, 0.02, 0.01, and 0.005$ among
populations. {\sc flock} was run nine times at $K=5$ with the default setting for all 
run parameters (initial partition = random, number of iterations = 20, number of runs = 50, 
LLOD threshold = 0). The default model in {\sc structure}, the no-admixture and correlated
allele frequency (NA-C) model, the no-admixture and non-correlated allle frequency (NA-NC) model, 
and the no-admixture and non-correlated allele frequency (NA-NC) model 
were run nine times with a 5,000 sweep burn-in followed by a 20,000 sweep sample 
from the posterior. All runs were performed on a JNCS D685 with an i7 3.40GHz processor and 8GB
...
of individuals. 
\citet{Duc&Tur2012} developed two stopping rules that they use to estimate $K$. 
...
%This section may get cut depending on how much we want to talk about estimating K.
...
non-trivial. {\sc flock} averaged 32.8 minutes
for each run of the first simulated dataset, while each group of 
9 iterations for {\sc structure} averaged 14.7, 15.3 and 18.7 minutes
for the NA-NC, NA-C, and A-C models respectively. 
...
between populations the solutions that {\sc flock} found were more variable.
Of the nine runs of {\sc flock} for the first simulated dataset, between 0 and 22
differences between the final arrangement of indivuals within the ($K=5$)
clusters. Of the nine runs, four pairs of runs found the 
same solution (runs 1 \& 2, 5 \& 6, 3 \& 7, and 4\&9). Run 8, which 
had no plateaus, was the most different with an average of 19.86 differences.
For the third simulated dataset, the number 
of differences ranged from 482 to 911.  {\sc structure} showed less variation for 
each set of simulations. Solutions for the first simulated dataset differed by the
placement of between 0 and 4 individuals and between 5 and 17 for the third 
dataset.
%Create a figure comparing all of the values. All the text is somewhat hard to read
 
Plateau lengths greater than 0 were only obsered for the first simulated dataset. For each
of the nine runs of {\sc flock} the plateau lengths varied from 0 to 4. In no analysis 
did we observe a plateau lenght greater than 6. Despite the similar allocation of individuals into $K$ clusters, the 
...
\begin{figure*}
\begin{center}
    \includegraphics[width=\textwidth]{images/Figures-Pat/Fig_AllSim.pdf}   % Use this if we want it color (I'll make greyscale later)  
...
%gray-scale
vertical bar. Each dataset is comprised of five populations of sizes 550, 550, 400, 250, and 250 individuals. The 
average pairwise $F_{ST}$ among populations was 0.03, 0.02, 0.01, and 0.005 for simulations 1, 2, 3, and 4 respecitively. 
}
    \label{fig:Sims}
\end{center}
\end{figure*}
...
\begin{figure*}
\begin{center}
    \includegraphics[width=0.5\textwidth]{images/Figures-Pat/Loss.pdf}
    \caption{Comparision of zero-One loss for the admixture and correlated allele frequency (A-C) model, 
the no-admixture and correlated allele frequency (NA-C) model, the no-admixture and non-correlated 
allle frequency (NA-NC) model, the no-admixture and non-correlated allele frequency (NA-NC) 
model in {\sc structure} and the {\sc flock} model for each of the four simulated datasets.}
    \label{fig:Loss}
\end{center}
\end{figure*}
