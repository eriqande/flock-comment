...
using the default settings required 1133 minutes to complete. Each individual group  of runs 
averaged 188.8 minutes (1.5 min SD). The six iterations of {\sc structure} completed in 
XXXX minutes with each iterations averaging XXXX (SD). 

While no  plateaus were observed in any of the six groups for a \textit{k} greater than 
two, the plateau record varied greatly among each
...
either an estimate of K$\geq$2 or \textit{undecided} as a consequence of no plateau 
lengths being greater than 6. Distruct plots for each group of runs for \textit{k}=2-4 
lead to nearly identical clustering of individuals (See supplementary materials). While 
only two of the six 'best runs' when 
\textit{k}=2 ended with identical clustering (same mean LLOD score), all other runs 
differed by at most the allocation of 13 individuals. When \textit{k} was set to three 
and four all runs provided very similar clustering. 
