...
conditional distribution of $Y_{i\ell}$ given $Z_i=k$, and $Z_{(-i)}$ and $Y_{(-i)
\ell}$---the
cluster memberships and genotypes, respectively,  of all the remaining individuals---
is
...
belong to cluster $k$. Calculating (\ref{eq:cdm}) for each value of $Z_i=k \in 
\{1,\ldots,K\}$ 
