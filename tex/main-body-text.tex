...
The allocation of indivividuals into K clusters is congruent for each program. For all six groups 
...
that of the 50 starting allocations of individuals in the {\sc flock} procedure only the LLOD for the 
 'best run' is reported. Of these 'best runs' for a \textit{k} of 2 only two  
...
one that differs from both the pure likelihood 
and delta K approach in {\sc structure}. While no plateaus were observed in any of the six groups 
...
lengths being greater than 6. Interpreting the results from {\sc structure} can either be done 
with the estimated likelihood of the data from the program itself or by the delta k method 
suggest by Evanno et al XXXXX. Both methods indicate that the most likely \textit{k} for the dataset is 6. 
