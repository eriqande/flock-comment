...
of runs for \textit{k} = 2 - 4, {\sc flock} resulted in nearly identical clustering of individuals 
...
the $\Delta K$ method supported a $K$ of 2. 
...
\begin{figure}
\begin{center}
	% NOTE. Pat replaced old Q-val-StrFloc.pdf with a ggplot version 
...
number of iterations did not affect the overall inference of \textit{k}. We 
...
by a 200,000 sweep sample from the posterior of \textit{k} from 1 to K+3. 
It seemed reasonable to compare, for each \textit{k}, one iteration of {\sc structure} 
to one iteration of {\sc flock} (50 different starting allocations that are reallocated 20 times) 
as both evaluate the partition space once. The dataset used in this comparision was an order of magnatude
larger than the largest used by \citep{Duc&Tur2012} (N = 60, 120, and 240). Any process speed advantages 
may only been observed with small sample sizes. 
...
which they can be partitioned. The total partition space can be 
...
The likelihood surface spanning all possible partitions
may be characterized by multiple peaks and troughs.
...
individuals as it has no way to visit areas of the likelihood surface with smaller values.
 It is not surprising that with 2,596 individuals we could have 
drastically different plateau sequences with just two clusters and that as \textit{k} increases
{\sc flock} finds suboptimal allocations. If the number of initial allocations does not increase
as $K$ is increased, the starting allocations become diffusely scattered in the space of all
possible partitions making it unlikely that they will converge.
...
to be cautious of interpreting K, much less effort has been put forth examining how the number of individuals
and thenumber of starting allocations can affect not only the convergence of different runs, but also the 
...
allocations will provide no support to a given \textit{k}.
