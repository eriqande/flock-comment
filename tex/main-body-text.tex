...
among its ``best runs'' at higher values, while still finding solutions that were similar to {\sc structure}'s.
...
The peak or plateau that  {\sc flock}'s deterministic algorithm finds is entirely dependent on the initial allocation of 
...
While the authors observed faster processing times (relative to other methods) in their 
...
by a 200,000 sweep sample from the posterior of $K$ from 1 to three more than the most likely 
value of $K$.
To us it seemed reasonable to compare, for each $K$, one iteration of {\sc structure} (50,000 burn in and 150,000 sample collection sweeps)
...
an apparent advantage in speed.  Regardless, while it may hold that {\sc flock}
is able to process small datasets quickly enough to compete with {\sc structure},
the rapid increase in our ability to economically
genotype a large number of individuals for a great many loci, may render this advantage less relevant to many 
...
we have provided a different prespective on the algorithm
